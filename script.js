...
function riskModel(simulations, lower, upper, confidence_level, events, reserve) {
  console.log("Running risk model..."); // Debugging step 1: Ensure the function is called
  ...
  // Combine distributions to calculate total loss
  let total_loss = [];
  for (let i = 0; i < simulations; i++) {
  ...
  let cvar = total_loss
    .filter(x => x > varValue)
    .reduce((a, b) => a + b, 0) / total_loss.filter(x => x > varValue).length;
  ...
  console.log("Simulation complete!"); // Debugging step 2: Ensure the function runs till the end
...
document.getElementById('riskForm').addEventListener('submit', function (event) {
  ...
  console.log("Form submitted!"); // Debugging step 3: Ensure the form submission event is triggered
  ...
  console.log(result); // Debugging step 4: Ensure the result object is correct
